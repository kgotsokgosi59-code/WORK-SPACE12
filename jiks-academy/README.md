...
| `skhumbuzo.html` | Profile — Talent Liaison & Community (crew branch) |

## Styles (4 files, one job each)
...
| `assets/css/pages.css` | Page-specific: hero, page heads, profiles, pager, site map, services, contact |
| `assets/css/cover.css` | **The cover page only** — linked from `index.html` alone, so no other page downloads it |
...
**Sizing** all lives in `cover.css`. Nothing is a fixed pixel size — it is `vmin` /
`vw` / `vh` inside `clamp()`, so the cover fills the screen instead of sitting small
in the middle. Two measured facts drive the numbers: the wordmark `JIKS ACADEMY` is
**5.23× the font-size wide** and the longest word, `ACADEMY`, is **3.45×**.

- **Landscape** (laptop, iPad sideways): the wordmark stays on **one line** at
  `min(15vw, 24vh)` — wide enough to fill, capped so it never wraps or overflows.
- **Portrait** (phone, iPad upright): it runs onto **two lines** at
  `min(20.5vw, 26vh)`, and the vertical spacing switches to `vh` to use the extra height.
- **Landscape phones** (`max-height: 620px`): the stack flips to a **two-column grid**,
  logo on the left, so it fits a 390px-tall screen.

On top of that, three fill multipliers are tuned **per class of screen** — `--cov`
(landscape), `--cov-p` (portrait) and `--cov-g` (landscape phones) — each set to the
largest value that still fits its own screens. Raise one to fill more, lower it if a
device reports clipping.

Measured in Chromium, Firefox and WebKit across **21 viewports** (320×568 up to
2560×1440), in both normal and reduced-motion modes — **126 combinations, zero
overflow**, with the composition filling **63–94% of the screen height**.

Two helpers keep this honest:
- `python3 cover_fit.py` — prints fill % and any overflow per viewport.
- `python3 tune_cover.py` — binary-searches each multiplier for the largest value that
  still fits, per screen class, in all three engines. Run it after changing the cover
  and bake the numbers it recommends into the `@media` bands in `cover.css`.
...
4. **Crew branch** — `crew.html` also offers the three profiles, and from there the
   chain runs `crew → Shadrack → Kgotso → Skhumbuzo → back to crew`. The profiles use
...
assets/img/crew/skhumbuzo.jpg  Skhumbuzo (4:5 crop, 900x1125)
```
...
4. **Crew roles** for Shadrack Makamu, Kgotso Matlakala and Skhumbuzo are written to
   fit the movements — confirm or correct them. Note the spelling: **Skhumbuzo**, with
   an "m" (the profile file is `skhumbuzo.html`).
...
5. **Kgotso's portrait** is a tall phone photo cropped by `object-position: center 20%`
   in `pages.css` — nudge that value if the crop misses. Skhumbuzo's is already cropped
   to the 4:5 the layout wants, so it needs no offset.
